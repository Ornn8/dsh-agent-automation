...
test('target recovery forwards the configured controller login through its reusable input', async () => {
  ...
  assert.match(controller, /controller_login:\s*\r?\n\s*required: true\s*\r?\n\s*type: string/)
...
test('snapshot collection uses full upstream history and stable exact Issue and pull request pairs', async () => {
  ...
  assert.match(snapshot, /loadRelevantWorkflowRuns\(\{ repository, config, environment \}\)/)
  assert.match(snapshot, /ACTIVE_WORKFLOW_RUN_STATUSES = \['queued', 'in_progress', 'requested', 'waiting', 'pending'\]/)
  assert.match(snapshot, /path: `repos\/\$\{repository\}\/actions\/runs\?per_page=100`/)
  assert.doesNotMatch(snapshot, /description: 'workflow runs', collection: 'workflow_runs'/)
