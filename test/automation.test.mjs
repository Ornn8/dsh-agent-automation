...
test('a completed repair routes review and landing through the same trusted Profile workflow', async () => {
  ...
  assert.match(advanceSource, /assertVerificationContractChecks/)
  assert.match(landSource, /assertVerificationContractChecks/)
  assert.match(advanceSource, /assertVerificationContractExecution/)
  assert.match(landSource, /assertVerificationContractExecution/)
  assert.match(advanceSource, /verificationJobId\(checkRun\.details_url, repository\)/)
  assert.match(landSource, /verificationJobId\(checkRun\.details_url, repository\)/)
  const independentChecks = landSource.match(/const independent = configured\.filter\(check => check\?\.context !== 'agent\/review'\)[\s\S]*?const names/)
  assert.ok(independentChecks, 'landing must inspect branch-protection checks before selecting contract names')
  assert.match(independentChecks[0], /configuredRequiredChecks: independent/)
