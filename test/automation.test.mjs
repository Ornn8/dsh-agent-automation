...
test('Worker dispatch paths derive routing from local trusted state', async () => {
  ...
  const repairSource = await readFile(new URL('../src/dsh-repair.mjs', import.meta.url), 'utf8')
  const issueSource = await readFile(new URL('../src/dsh-issue.mjs', import.meta.url), 'utf8')
  for (const source of [backlogSource, advanceSource, reviewSource, repairSource, issueSource]) {
    assert.doesNotMatch(source, /worker_routing_execution|WORKER_ROUTING_EXECUTION_JSON|createWorkerRoutingExecution/)
  }
