...
test('dispatch backlog provisions its control label before editing or dispatching', async () => {
  ...
  const requestIdStart = source.indexOf('const requestId = agentWorkRequestId(')
  const requestIdEnd = source.indexOf(')', requestIdStart)
  assert.ok(requestIdStart >= 0 && requestIdEnd > requestIdStart)
  const requestIdCall = source.slice(requestIdStart, requestIdEnd)
  assert.match(requestIdCall, /\bwork\.work\b/)
  assert.match(requestIdCall, /\bprofile\.definitionHash\b/)
  assert.match(requestIdCall, /\bprofile\.verificationContract\?\.hash\b/)
  assert.match(requestIdCall, /\brepository\b/)
  assert.match(requestIdCall, /\bwork\.number\b/)
