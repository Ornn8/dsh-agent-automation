...
test('repair Skill hands a proven CI baseline failure to a same-repository Issue', () => {
  ...
  assert.match(repairSkill, /Add `agent\/dsh` only after the Issue exists/)
  assert.match(repairSkill, /Finish the baseline path only after the open Issue has `agent\/dsh`/)
  ...
  assert.match(repairSkill, /pull request comment must not be used to authorize or route the new work/)
