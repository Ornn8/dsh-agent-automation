...
const PROJECTION_FIELDS = new Set([
  'version', 'workRequestId', 'role', 'profileId', 'workflowId', 'stageId', 'definitionHash',
  'revision', 'subject', 'routeDecision', 'capacityGenerationHash', 'observationId',
])
...
function positiveNumber(value, field) {
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 1) throw new Error(`CapacityWaitProjection ${field} is invalid`)
...
function parseSubject(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('CapacityWaitProjection subject is invalid')
  const subject = /** @type {AnyObject} */ (value)
  for (const key of Object.keys(subject)) if (!SUBJECT_FIELDS.has(key)) throw new Error(`CapacityWaitProjection subject has unknown field ${key}`)
...
function parseRevision(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('CapacityWaitProjection revision is invalid')
  const revision = /** @type {AnyObject} */ (value)
  for (const key of Object.keys(revision)) if (!REVISION_FIELDS.has(key)) throw new Error(`CapacityWaitProjection revision has unknown field ${key}`)
  return {
...
function parseRouteDecision(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('CapacityWaitProjection routeDecision is invalid')
  const route = /** @type {AnyObject} */ (value)
  try {
    return parseWorkerRouteDecision(route)
  } catch (error) {
...
/** Parse and validate one strict CapacityWaitProjection v1. */
/** @param {unknown} value @returns {AnyObject} */
export function parseCapacityWaitProjection(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('CapacityWaitProjection must be an object')
  const projection = /** @type {AnyObject} */ (value)
  for (const key of Object.keys(projection)) if (!PROJECTION_FIELDS.has(key)) throw new Error(`CapacityWaitProjection has unknown field ${key}`)
  for (const key of PROJECTION_FIELDS) if (!Object.hasOwn(projection, key)) throw new Error(`CapacityWaitProjection is missing ${key}`)
  ...
  const role = ROLES.has(text(projection.role)) ? text(projection.role) : (() => { throw new Error('CapacityWaitProjection role is invalid') })()
...
/** Create a strict CapacityWaitProjection v1. */
...
/** Parse exactly one CapacityWaitProjection status line from a controller comment. */
/** @param {unknown} body @returns {AnyObject} */
