...
/**
 * Return the stable identity for one capacity resume observation.
 * @param {AnyObject} value
 * @returns {string}
 */
export function capacityResumeRequestId(value = {}) {
  const subject = value.subject && typeof value.subject === 'object'
    ? /** @type {AnyObject} */ (value.subject)
    : null
  const workRequestId = identifier(value.workRequestId, REQUEST_ID, 'resume workRequestId')
  const subjectStateVersion = identifier(
    value.subjectStateVersion ?? subject?.stateVersion,
    FULL_SHA256,
    'resume subjectStateVersion',
  )
  const capacityGenerationHash = identifier(
    value.capacityGenerationHash,
    FULL_SHA256,
