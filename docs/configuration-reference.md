...
Task classification is a separate trusted policy input to `classifyWorkRequest`; it does not add fields to the machine-local Worker registry or the worker-neutral WorkRequest. A policy names bounded route classes and one `default` route, then may provide rules over trusted labels, paths, extensions, Workflow Stage, title/body excerpts, and controller-verified failure evidence. The Worker derives the selected class plus stable policy and evidence hashes from the live WorkRequest and subject snapshot on the target machine, then persists a local `WorkerRouteDecision v1` keyed by those hashes. A missing policy uses the explicit compatible default only when the local configuration has one Worker; a malformed policy, missing subject state, or forged transported payload fails closed. The decision never names a Worker, Adapter, provider, model, or credential, and capacity failover never changes its task class or exact subject state.
