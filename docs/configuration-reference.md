...
Task classification is a separate trusted policy input to `classifyWorkRequest`; it does not add fields to the machine-local Worker registry or the worker-neutral WorkRequest. A policy names bounded route classes and one `default` route, then may provide rules over trusted labels, paths, extensions, Workflow Stage, title/body excerpts, and controller-verified failure evidence. The classifier returns the selected class plus stable policy and evidence hashes bound to the WorkRequest id, role, and exact subject state version. An optional classifier may return only a configured class and must fall back to `default` for malformed, unavailable, asynchronous, or low-confidence output. Decision creation rejects a classification from another request or subject state. The resulting `WorkerRouteDecision v1` is transported only inside the controller-owned `worker_routing_execution` envelope, which also binds a trusted dispatch or recovery generation. Consumers revalidate it against the WorkRequest, live subject state, and current role routing policy before candidate resolution; an arbitrary or stale object is rejected. The PR4 role runner keeps that decision worker-neutral while resolving admitted candidates and consulting the local CapacityRecord registry; capacity failover never changes the task class or exact subject state.
