...
Task classification is a separate trusted policy input to `classifyWorkRequest`; it does not add fields to the machine-local Worker registry or the worker-neutral WorkRequest. A policy names bounded route classes and one `default` route, then may provide rules over trusted labels, paths, extensions, Workflow Stage, title/body excerpts, and controller-verified failure evidence. The classifier returns the selected class plus stable policy and evidence hashes. An optional classifier may return only a configured class and must fall back to `default` for malformed, unavailable, asynchronous, or low-confidence output. The resulting `WorkerRouteDecision v1` is durable only when stored by its caller and must be revalidated against the WorkRequest and exact subject state before later candidate resolution; PR2 does not activate that runtime path.
