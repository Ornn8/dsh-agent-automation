...
`roles` has exactly `change`, `review`, and `maintenance`. Each role requires `workers`; every role accepts one through eight Workers. A Worker cannot appear in two roles. The change and review pools are admission allowlists; PR4-B activates bounded runtime failover and PR5 persists and resumes capacity waits.
...
`routing.change.routes` and `routing.review.routes` are bounded named route maps. Each role routing object may set `maxCandidates` from 1 through 8 (default 8). The `routes` map contains named route objects, and every route has one through sixteen ordered `selectors`; a selector is exactly one of `{ "worker": "id" }`, `{ "allTags": ["tag"] }`, or `{ "route": "other-route" }`. Worker selectors name an admitted Worker id exactly, including ids accepted before routing existed. Tags are unique and matched with ordinal, case-sensitive equality; tag selectors order matching Worker ids by ordinal code-unit order. Route references must be acyclic and are compiled once per resolution with shared subgraphs memoized. Every non-default route must resolve to at least one admitted Worker, and review routes may contain only Workers with hard read-only isolation. When routing is omitted, the loader derives a `default` route selecting the first role Worker. Maintenance review executes only the first candidate from the review `default` route. Candidate resolution is deterministic; PR4-B uses it for bounded failover and PR5 revalidates the same decision when resuming a capacity wait.

Task classification is a separate trusted policy input to `classifyWorkRequest`; it does not add fields to the machine-local Worker registry or the worker-neutral WorkRequest. A policy names bounded route classes and one `default` route, then may provide rules over trusted labels, paths, extensions, Workflow Stage, title/body excerpts, and controller-verified failure evidence. The classifier returns the selected class plus stable policy and evidence hashes bound to the WorkRequest id, role, and exact subject state version. An optional classifier may return only a configured class and must fall back to `default` for malformed, unavailable, asynchronous, or low-confidence output. Decision creation rejects a classification from another request or subject state. The resulting `WorkerRouteDecision v1` is durable only when stored by its caller and must be revalidated against the WorkRequest and exact subject state before later candidate resolution; PR5 uses that revalidation for capacity-wait resume.
