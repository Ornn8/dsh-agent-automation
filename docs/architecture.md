...
- Product workflows select only the immutable `change` or `review` role. The local controller resolves every Agent from the single `operations.roles.<role>.workers` table. Controller maintenance uses the finite ordered `maintenance` list, while maintenance review uses the independent `review` role; a Worker cannot belong to more than one trust domain.
