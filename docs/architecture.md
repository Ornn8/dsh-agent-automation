...
- Issue creation, reopening, or editing queues change work only when the live trusted Issue contains one strict ready `agent-work:v1` declaration. Open dependencies defer dispatch, and dependency closure reevaluates the backlog. The worker rereads each dependency and rejects a queued canonical hash when the declaration changed or disappeared before execution.
