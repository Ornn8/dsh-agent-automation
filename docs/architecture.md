...
- Workflows select only the immutable `change` or `review` role. The local controller resolves every Agent from the single `operations.roles.<role>.workers` table and rejects missing, reused, or unknown role bindings before invoking an adapter.
